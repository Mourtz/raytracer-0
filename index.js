...
const bool U_EUCLIDEAN = true;
const bool U_SPHERE    = true;
const bool U_PLANE     = true;
const bool U_BOX       = true;
const bool U_SDF       = false;

const lowp int NUM_MESHES = 8;
const lowp int NUM_SDFS   = 0;
const lowp int NUM_MODELS = 0;

const Mesh meshes[NUM_MESHES + NUM_SDFS + NUM_MODELS] = Mesh[](
    // Floor  (y = -1.5, normal up)
    Mesh(MAT_CORNELL_WHITE, PLANE,  vec3( 0.0, 1.0, 0.0), vec4(1.5, 0.0, 0.0, 0.0)),
    // Ceiling (y = +1.5, normal down)
    Mesh(MAT_CORNELL_WHITE, PLANE,  vec3( 0.0,-1.0, 0.0), vec4(1.5, 0.0, 0.0, 0.0)),
    // Back wall (z = -2.5)
    Mesh(MAT_CORNELL_WHITE, PLANE,  vec3( 0.0, 0.0, 1.0), vec4(2.5, 0.0, 0.0, 0.0)),
    // Left wall — red
    Mesh(MAT_CORNELL_RED,   PLANE,  vec3( 1.0, 0.0, 0.0), vec4(1.5, 0.0, 0.0, 0.0)),
    // Right wall — green
    Mesh(MAT_CORNELL_GREEN, PLANE,  vec3(-1.0, 0.0, 0.0), vec4(1.5, 0.0, 0.0, 0.0)),
    // Ceiling light (index 5) — r=0.3, sits just below ceiling
    Mesh(MAT_LIGHT_4,       SPHERE, vec3( 0.0, 1.4,-1.2), vec4(0.3, 0.0, 0.0, 0.0)),
    // Tall box — right side, resting on floor (half-size 0.5 → bottom at y=-1.5)
    Mesh(MAT_CORNELL_WHITE, BOX,    vec3( 0.5,-1.0,-1.8), vec4(1.0, 0.0, 0.0, 0.0)),
    // Short box — left side, resting on floor (half-size 0.35 → bottom at y=-1.5)
    Mesh(MAT_CORNELL_WHITE, BOX,    vec3(-0.45,-1.15,-1.3), vec4(0.7, 0.0, 0.0, 0.0))
);

// Single overhead light
const lowp int light_index[1] = int[](5);`;

    this.sdf_meshes = []; // No SDF meshes in this scene

    this.camera = {
      "origin": new Vector3(0.0, 0.0, 2.8),
